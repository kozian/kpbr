...
9. Нажимаем `Save` (`Сохранить`), потом `Save & Apply` (`Применить`). 
10. Переходим в меню `Network` (`Сеть`) > [Firewall (Межсетевой экран)](http://192.168.1.1/cgi-bin/luci/admin/network/firewall).
11. В нижней секции `Zones` (`Зоны`) в строке с созданной нами `awg` зоной выбираем `Edit` (`Изменить`)
   - Устанавливаем галочку `IPv4 Masquerading` (`Маскарадинг IPv4`)
   - Устанавливаем галочку `MSS clamping` (`Ограничение MSS`)
...
**! Важно**: установка kpbr определяет WAN интерфейс по default маршруту. Если планируете устанавливать его, отложите включение маршрутизации `Route Allowed IPs` пока его не установите.
